...
class Monster(Item):
    ...
    def benutzen(self, held):
        ...
        while held.getkampfwerte()[2] > 0 and self._kampfwerte[2] > 0:
            ...
            heldle = 0
            # Held schlaegt zu
            if random.randint(1, 20) <= held.getkampfwerte()[0] + held.getwaffe().getmod()[0] and random.randint(1,
                                                                                                                 20) > \
            ...
            # Monster schlaegt zu
            if self._kampfwerte[2] > 0 and random.randint(1, 20) <= self._kampfwerte[0] + self._mod[
                0] and random.randint(1, 20) > held.getkampfwerte()[1] + held.getwaffe().getmod()[1]:
                tp = self._tp[1]
